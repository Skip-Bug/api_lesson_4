...
def time_parser():
    """Дает возможность выбирать задержку и первое фото публикаций."""
    ...
    parser.add_argument(
        '-p', '--photo', help='Выбрать фото для старта')
    return parser
...
def main():
    ...
    если нет отправляет случайное изображение.
    Затем переходит к бесконечному циклу отправки всех фото из папки images.
    Задержка между отправками задаётся первым аргументом
    (в секундах, по умолчанию 14400).
